...
?>
<div class="header-modals-small <?php if ($arFavor) : ?>not-empty<?php else : ?>empty<?php endif; ?>" data-fav-modal="data-fav-modal" data-header-modal="5" data-type="favor-header">
    <?php if ($favorAjax) {
    ...
    <?php if ($arFavor) : ?>
        ...
        <div class="header-modals-small__bottom">
            <div class="header-modals-small__fav-count">
                <span><?= count($arFavor); ?> </span>&nbsp;<?= $arFavorInfo['count_cur']; ?> в избранном
            </div>
            <a class="button button--brown-border" href="/favorites/">Перейти в избранное</a>
        </div>
    <?php else : ?>
        <div class="header-modals-small__ttl">Избранное</div>
        <div class="header-modals-small__txt">Вы не добавили товары в избранное. Перейдите в каталог, чтобы добавить товары</div>
