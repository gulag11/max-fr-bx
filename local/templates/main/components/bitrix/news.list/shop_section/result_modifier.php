...
$arResult['CUR_SECT'] = $arResult['SECTION']['PATH'][array_key_last((array)$arResult['SECTION']['PATH'])];
...
if ($arParams['PHP_FILTER']) {
    ...
    while ($arData = $rsData->GetNextElement()) {
        ...
        if ($check) {
            ...
            foreach ($itemProps as $code => $propVal) {
                if (!in_array($propVal, array_column((array)$arResult['FILTERS'][$code]['VALUES'], 'VALUE'))) {
                    $arResult['FILTERS'][$code]['VALUES'][] = [
                        'VALUE' => $propVal,
                        'COUNT' => 1,
                    ];
                } else {
                    $valueKey = array_search($propVal, array_column((array)$arResult['FILTERS'][$code]['VALUES'], 'VALUE'));
    ...
    while ($arData = $rsData->GetNextElement()) {
        ...
        if (!in_array((int)$fields['CATALOG_PRICE_1'], array_column((array)$arResult['FILTERS']['PRICE']['VALUES'], 'VALUE'))) {
            $arResult['FILTERS']['PRICE']['VALUES'][] = [
                'VALUE' => (int)$fields['CATALOG_PRICE_1'],
                'COUNT' => 1,
            ];
        ...
        foreach ($props as $code) {
            ...
            if (!in_array($prop['VALUE'], array_column((array)$arResult['FILTERS'][$prop['CODE']]['VALUES'], 'VALUE'))) {
                $arResult['FILTERS'][$prop['CODE']]['VALUES'][] = [
                    'VALUE' => $prop['VALUE'],
                    'COUNT' => 1,
                ];
            } else {
                $arResult['FILTERS'][$prop['CODE']]['VALUES'][array_search($prop['VALUE'], array_column((array)$arResult['FILTERS'][$prop['CODE']]['VALUES'], 'VALUE'))]['COUNT']++;
            }
